...
- AI invite cards: `runInviteCardPipeline` (`services/aiInviteCardGeneration.service.ts`, model `gemini-3-pro-image`, 9:16) runs DESIGN (text-less artwork, saved to `invite_design_image_url` with `design_fingerprint`, a hash of the built stage 1 prompt plus its image keys) then TYPESETTING; an attempt skips DESIGN when the stored fingerprint still matches, so job retries, "Try again" and text-only edits reuse the artwork instead of redrawing. Prompts are assembled in `utils/*PromptBuilder*`/`*Catelogue*` utils. Every Gemini call goes through `generateImage` (`utils/geminiImage.util.ts`: 120s abort per call, in-call retry for `OVERLOADED` only, failures classified into `GENERATION_ERROR_CODE` and stored as `generation_error_code` for the UI to explain). The queue allows 3 attempts with 30s/90s backoff for retryable codes; the job throws `UnrecoverableError` for the rest (billing, safety, invalid input), and a card is COMPLETED only once typesetting has saved a final image. Source images are normalised with sharp (`utils/imageNormalize.util.ts`: EXIF rotate, longest side 2048px, JPEG/PNG with a truthful mime type). The worker writes `generation_heartbeat_at` at every stage and retry, and the status endpoint retires cards silent for 10 minutes. `faceSwap.service.ts` is unused.
